  ...
  <div class="login-contain">
    ...
    <div class="form-group">
      <div class="form-item">
        ...
        <input id="username" type="text" placeholder="请输入账号" autocomplete="new-password" v-model="state.phone">
      </div>
      <div class="form-item">
        <label for="password">
          <img src="../assets/img/password.png" alt="">
        </label>
        <input id="password" type="password" placeholder="请输入密码" autocomplete="new-password" v-model="state.password">
      </div>
    </div>
...
export default {
    setup() {
        ...
        async function login() {
            let res = await store.dispatch('phoneLogin', {phone: state.phone, password: state.password});
            if(res.data.code == '200') {
                // $router.push('/me) setup如何拿到router对象？？
            }
        }
        return {
            state,
